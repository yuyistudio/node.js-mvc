...
require(__dirname + '/refLib/shotenjin');
...
exports.runServer = function(port, config){
    if (!port || !config){
        console.err("port & config expected");
        return;
    }
    var server = http.createServer(function(req, res){
        var method = req.method.toUpperCase();
        if (method == 'GET') {
          handlerRequest(req, res, config);
        } else if (method == 'POST') {
          var _postData = '';
          ...
          req.end('unsupport method: ' + method.toString());
        }
    }).listen(port);
 ...
 * 所有请求的统一入口
 */
var handlerRequest = function(req, res, config){
    ...
    if(actionInfo.action){
        ...
        var controller = require('../controller/' + actionInfo.controller); // ./controllers/blog
        ...
        staticFileServer(req, res, config);
    }
};
...
var viewEngine = {
    render: function(req, res, viewName, context){
        var filename = path.join('./view', viewName);
        try{
...
var staticFileServer = function(req, res, config, filePath){
    if(!filePath){
        filePath = path.join('./', config.staticFileDir, url.parse(req.url).pathname);
    }
